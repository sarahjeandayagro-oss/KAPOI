...
@section('title', 'Panabo City Library - Admin Portal')
...
<section id="panel-dashboard" class="panel active dashboard-stats-section">
    <div class="stats-grid">
     <!-- @include('components.stat-card', ['label' => 'Pending Verifications', 'value' => count($pendingUsers), 'change' => 'Awaiting approval', 'tone' => 'gray', 'icon' => '🕒']) -->
      ...
      @include('components.stat-card', ['label' => 'Total Visitors (' . ($currentMonthName ?? now()->format('F')) . ')', 'value' => $totalVisitorsThisMonth ?? 0, 'change' => 'This month', 'tone' => 'brown', 'icon' => '👣'])
    </div>
    ...
    <div class="operations-summary">
      ...
      <div class="operations-grid">
        <a href="#" onclick="showPanel('borrow-requests'); return false;" class="operations-item urgent">
          ...
          <strong>{{ $pendingBorrowRequests->count() }}</strong>
    ...
    {{-- COLLECTION OVERVIEW + OVERDUE BOOKS ROW --}}
        ...
        <div class="card-body">
          <div class="collection-stats">
            ...
            <div class="collection-total-card">
              <div class="collection-total-num">{{ number_format($totalBooks) }}</div>
            ...
            <div class="collection-bars">
              <div class="collection-bar-item">
                <div class="collection-bar-header">
                  ...
                  <span class="collection-bar-val">{{ $availableBooks }}</span>
                </div>
                <div class="collection-bar-track"><div class="collection-bar-fill" style="width:{{ $availablePct }}%; background:#22c55e;"></div></div>
              </div>
              <div class="collection-bar-item">
                <div class="collection-bar-header">
                  <span class="collection-bar-dot" style="background:#3b82f6;"></span>
                  <span class="collection-bar-label">Issued</span>
                  <span class="collection-bar-val">{{ $issuedBooks }}</span>
                </div>
                <div class="collection-bar-track"><div class="collection-bar-fill" style="width:{{ $issuedPct }}%; background:#3b82f6;"></div></div>
              </div>
              <div class="collection-bar-item">
                <div class="collection-bar-header">
                  <span class="collection-bar-dot" style="background:#f59e0b;"></span>
                  <span class="collection-bar-label">Reserved</span>
                  <span class="collection-bar-val">{{ $reservedBooks }}</span>
                </div>
                <div class="collection-bar-track"><div class="collection-bar-fill" style="width:{{ $reservedPct }}%; background:#f59e0b;"></div></div>
              </div>
              <div class="collection-bar-item">
                <div class="collection-bar-header">
                  <span class="collection-bar-dot" style="background:#ef4444;"></span>
                  <span class="collection-bar-label">Lost / Damaged</span>
                  <span class="collection-bar-val">{{ $lostDamagedBooks }}</span>
                </div>
                <div class="collection-bar-track"><div class="collection-bar-fill" style="width:{{ $lostPct }}%; background:#ef4444;"></div></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="card admin-overdue-books">
        <div class="card-header">
          ...
          @if($overdueBooksList->isNotEmpty())
            <span class="overdue-count-badge">{{ $overdueBooks }}</span>
          @endif
        </div>
        <div class="card-body">
          @forelse($overdueBooksList as $overdue)
            @php $daysOverdue = \Carbon\Carbon::parse($overdue->due_date)->diffInDays(now()); @endphp
            <div class="overdue-item">
              <div class="overdue-item-left">
                <div class="overdue-borrower">{{ $overdue->borrower_name }}</div>
                <div class="overdue-book-title">{{ $overdue->book_title }}</div>
                <div class="overdue-meta">
                  <span class="overdue-days {{ $daysOverdue > 14 ? 'critical' : ($daysOverdue > 7 ? 'warning' : '') }}">{{ $daysOverdue }} {{ Str::plural('day', $daysOverdue) }} overdue</span>
                  <span class="overdue-due">Due {{ \Carbon\Carbon::parse($overdue->due_date)->format('M d, Y') }}</span>
                </div>
              </div>
    ...
    {{-- RECENT ACTIVITY + BORROWING ACTIVITY ROW --}}
    <div class="grid-2 admin-split-cards">
      {{-- RECENT ACTIVITY --}}
      @component('components.card', ['title' => 'Recent Activity', 'subtitle' => 'Latest library actions'])
        <div class="activity-list-redesigned">
          @forelse($recentActivity as $activity)
            ...
            <div class="activity-item-redesigned">
              <div class="activity-icon-circle" style="background:{{ $actColor }}1a; color:{{ $actColor }};">{{ $actIcon }}</div>
              <div class="activity-body-redesigned">
                <div class="activity-text-redesigned">{!! $activity->text !!}</div>
                <div class="activity-time-redesigned">{{ $actTimestamp }}</div>
              </div>
            </div>
          @empty
      ...
      {{-- BORROWING ACTIVITY / GENDER DISTRIBUTION --}}
      @if(!empty($monthlyBorrowing) && array_sum($monthlyBorrowing) > 0)
        @component('components.card', ['title' => 'Borrowing Activity', 'subtitle' => 'Borrowed books by month (' . now()->year . ')'])
          <div class="borrowing-activity-chart">
            @php $maxBorrow = max(max($monthlyBorrowing ?: [0]), 1); @endphp
            @foreach($monthlyBorrowing as $monthNum => $count)
              @php
              ...
              @endphp
              <div class="ba-bar-col" title="{{ $monthAbbr }}: {{ $count }}">
                <div class="ba-bar-val">{{ $count }}</div>
                <div class="ba-bar {{ $isCurrentMonth ? 'current' : '' }}" style="height:{{ max($barHeight, $count > 0 ? 8 : 4) }}px;"></div>
                <div class="ba-bar-label">{{ $monthAbbr }}</div>
              </div>
            @endforeach
          </div>
        @endcomponent
      @else
        @component('components.card', ['title' => 'Gender Distribution', 'subtitle' => 'Registered user demographics'])
          <div class="gender-table-card">
            <div class="gender-table-row">
              ...
              <div class="gender-table-bar">
                <span style="width: {{ $femalePct ?? 0 }}%; background: #ec4899;"></span>
              </div>
              <div class="gender-table-metric">
                <strong>{{ $femalePct ?? 0 }}%</strong>
                <small>({{ $femaleCount ?? 0 }})</small>
              </div>
            </div>
            <div class="gender-table-row">
              <div class="gender-table-label">
              ...
              <div class="gender-table-bar">
                <span style="width: {{ $malePct ?? 0 }}%; background: #3b82f6;"></span>
              </div>
              <div class="gender-table-metric">
                <strong>{{ $malePct ?? 0 }}%</strong>
                <small>({{ $maleCount ?? 0 }})</small>
              </div>
            </div>
          </div>
        @endcomponent
          ...
          @forelse($recentBorrowings as $borrow)
            <div class="recent-borrowing-item" style="display:flex; align-items:center; gap:12px;">
              ...
              <div style="flex:1; min-width:0;">
                <div class="recent-borrowing-book" style="font-weight:700; color:var(--text);">{{ $borrow->title ?? 'Unknown' }}</div>
                <div class="recent-borrowing-meta" style="margin-top:4px; color:var(--muted);">{{ $borrow->borrower_name ?? $borrow->member_name ?? 'Member' }}</div>
                @if(!empty($borrow->due_date))
                  <div class="recent-borrowing-meta" style="margin-top:4px; color:var(--muted);">
                    <strong>Due:</strong> {{ \Carbon\Carbon::parse($borrow->due_date)->format('M d, Y') }}
                  </div>
                @endif
              </div>
              <div class="recent-borrowing-status" style="white-space:nowrap; text-align:right;">
                {{ $borrow->accession_number ?? $borrow->isbn ?? $borrow->barcode ?? '—' }}
              </div>
            </div>
          @empty
  ...
  <section id="panel-verified" class="panel">
    @component('components.card', ['title' => 'Verified Accounts', 'subtitle' => 'Approved student, researcher, and visitor accounts'])
      @slot('actions')
        ...
        <tbody>
          @forelse($verifiedUsers as $user)
            <tr>
              ...
              <td>{{ $user->barcode_id ?? 'Unassigned' }}</td>
              <td>{{ $user->school ?? 'N/A' }}</td>
              <td>{{ ucfirst($user->role) }}</td>
              <td><span class="status-chip approved">Approved</span></td>
              <td>{{ $user->updated_at ? \Carbon\Carbon::parse($user->updated_at)->format('M d, Y h:i A') : 'N/A' }}</td>
          ...
          <table id="adminBooksTable">
            ...
            <tbody>
              @if(\Illuminate\Support\Facades\Schema::hasTable('books'))
                @forelse(\Illuminate\Support\Facades\DB::table('books')->whereNull('archived_at')->get() as $book)
                  <tr>
                    <td><code>{{ $book->barcode }}</code></td>
                    <td><code>{{ $book->call_number ?? 'N/A' }}</code></td>
                    <td><strong>{{ $book->title }}</strong><br><small style="color:var(--text); opacity:0.6;">Accession: #{{ $book->accession_number ?? '000' }} | Location: {{ $book->section_location ?? 'General' }}</small></td>
                    <td>{{ $book->author }}</td>
                    <td><span class="status-chip borrowed" style="background:rgba(66,86,131,0.06); color:var(--text);">{{ $book->category }}</span></td>
                    <td>{{ $book->available }} / {{ $book->copies }} Available</td>
                    <td>
                      @if($book->available > 0) <span class="status-chip returned">Available</span>
                      @else <span class="status-chip overdue">All Out</span> @endif
                    </td>
                    <td>
                      <form action="{{ route('admin.books.update', $book->id) }}" method="POST" style="display:grid; gap:6px; min-width:240px;">
                        @csrf
                        ...
                        <input type="hidden" name="section_location" value="{{ $book->section_location }}">
                        <input type="hidden" name="isbn" value="{{ $book->isbn }}">
                        <input type="hidden" name="summary" value="{{ $book->summary }}">
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:6px;">
                          <div>
                            <small style="color:var(--muted);">Copies</small>
                            <input type="number" name="copies" min="1" value="{{ $book->copies }}" style="width:100%;">
                          </div>
                          <div>
                            <small style="color:var(--muted);">Available</small>
                            <input type="number" name="available" min="0" value="{{ $book->available }}" style="width:100%;" title="Auto-calculated: copies minus borrowed. Server will enforce the correct value.">
                          </div>
                        </div>
                      ...
                      <form action="{{ route('admin.books.delete', $book->id) }}" method="POST" style="margin-top:6px;" class="confirmable" data-confirm="Archive this book?">
                        @csrf
            ...
            <tr>
              ...
              <td>
                <div style="display:flex; flex-wrap:wrap; gap:6px; align-items:flex-start;">
                  ...
                  <form method="POST" action="{{ route('staff.barcode-request.archive', $request->id) }}" style="display:inline;">
                    @csrf
        ...
        <tbody>
          @forelse($approvedBarcodeRequests as $request)
            <tr>
              ...
              <td>{{ $request->old_barcode_id ?? '—' }}</td>
              <td><strong>{{ $request->new_barcode_id ?? '—' }}</strong></td>
              <td style="max-width:220px; white-space:pre-wrap;">{{ $request->staff_notes ?? 'Approved' }}</td>
              <td>{{ \Carbon\Carbon::parse($request->created_at)->format('M d, Y h:i A') }}</td>
            </tr>
          @empty
  ...
  <div class="content">
            ...
            <tbody>
              @forelse($pendingUsers as $pendingUser)
                <tr>
                  ...
                  <td>{{ strtolower($pendingUser->role) === 'visitor' ? '' : ($pendingUser->school ?: 'N/A') }}</td>
                  ...
                  <td>
                    <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                      <form method="POST" action="{{ route('admin.verify-researcher') }}" class="verify-form">
                        @csrf
                        <input type="hidden" name="lookup" value="{{ $pendingUser->user_id }}">
            ...
            <tbody>
              @forelse($verifiedUsers as $verifiedUser)
                <tr>
                  ...
                  <td>{{ ucfirst($verifiedUser->role)}} </
                  <td>{{ $verifiedUser->updated_at ? \Carbon\Carbon::parse($verifiedUser->updated_at)->format('M d, Y h:i A') : 'N/A' }}</td>
                  <td>
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                      @if(empty($verifiedUser->barcode_id))
                        <form method="POST" action="{{ route('admin.generate-barcode') }}">
                          @csrf
                          <input type="hidden" name="user_id" value="{{ $verifiedUser->user_id }}">
            ...
            <tbody>
              @forelse($availableBarcodes as $barcode)
                <tr>
                  ...
                  <td>
                    <form method="POST" action="{{ route('admin.assign-barcode') }}" style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                      @csrf
    ...
    <div id="panel-users" class="panel">
          ...
          <table class="user-records-table">
            ...
            <tbody>
              @forelse($userRows as $userRow)
                ...
                @endphp
                <tr>
                  <td class="user-record-id">{{ $displayValue($userRow->user_id ?? null, 'N/A') }}</td>
                  <td class="user-record-name"><strong>{{ $displayValue($userRow->name ?? null, 'Unnamed User') }}</strong></td>
                  <td><span class="status-chip borrowed">{{ $roleLabel }}</span></td>
                  <td>{{ $displayValue($userRow->email ?? null, 'Not provided') }}</td>
                  <td><span class="status-chip {{ $statusClass }}">{{ $statusLabel }}</span></td>
                  <td class="user-record-muted">{{ $status === 'rejected' ? $displayValue($userRow->rejection_reason ?? null, 'No reason provided.') : '-' }}</td>
                  <td>
                    @if($unpaid > 0)
                      <span class="status-chip overdue">PHP {{ number_format($unpaid, 2) }} Due</span>
                    @else
                  ...
                  <td class="user-record-actions-cell">
                    ...
                    <a class="table-action-btn edit" href="{{ route('admin.users.edit', $userRow->id) }}"><x-icons.edit /> <span>Edit</span></a>
                    <form method="POST" action="{{ route('admin.users.delete', $userRow->id) }}">
                      @csrf
                      <button class="table-action-btn delete" type="submit" onclick="return confirm('Are you sure you want to delete this user?')"><x-icons.trash /> <span>Delete</span></button>
                    </form>
                    </div>
                  </td>
                </tr>
          ...
          <table>
            ...
            <tbody>
              @forelse($entryLogs as $log)
                <tr>
                  <td>{{ $log->full_name ?? 'Unknown Member' }}</td>
                  <td>{{ $log->barcode_id ?? 'N/A' }}</td>
                  <td>Member</td>
                  <td>{{ \Carbon\Carbon::parse($log->entry_time)->format('h:i A') }}</td>
                  <td>{{ $log->exit_time ? \Carbon\Carbon::parse($log->exit_time)->format('h:i A') : 'Active' }}</td>
      ...
      <div class="card" style="margin-top:24px;">
        ...
        <div class="card-body">
          <form action="{{ url('/admin/certificates/generate') }}" method="POST" style="display:flex; gap:12px; align-items:end;">
            @csrf
    ...
    <div id="panel-borrowing" class="panel">
      ...
      <div class="card" style="margin-bottom:18px;gap:8px;">
        ...
        <div class="card-body">
          <form action="{{ route('admin.borrow-by-barcode') }}" method="POST" style="display:grid; gap:16px;">
            @csrf
      ...
      <div class="card">
        <div class="card-body" style="padding:0;">
          <table id="borrowingLifecycleTable">
            ...
            <tbody>
              @if(\Illuminate\Support\Facades\Schema::hasTable('borrowing_transactions'))
                @forelse(\Illuminate\Support\Facades\DB::table('borrowing_transactions')->leftJoin('books', 'borrowing_transactions.book_id', '=', 'books.id')->select('borrowing_transactions.*', 'books.title')->latest('borrowing_transactions.created_at')->get() as $borrow)
                  <tr>
                    <td>TXN-{{ str_pad($borrow->id, 4, '0', STR_PAD_LEFT) }}</td>
                    <td>{{ $borrow->barcode_id }}</td>
                    <td>{{ $borrow->title ?? $borrow->book_barcode }}</td>
                    <td>{{ \Carbon\Carbon::parse($borrow->borrow_date)->format('M d, Y') }}</td>
                    <td>{{ \Carbon\Carbon::parse($borrow->due_date)->format('M d, Y') }}</td>
                    <td><span class="status-chip {{ $borrow->status === 'Returned' ? 'returned' : 'borrowed' }}">{{ $borrow->status }}</span></td>
                    <td>
                      @if($borrow->status !== 'Returned')
                        <div style="display:grid; gap:8px;">
                          ...
                          @if(\Carbon\Carbon::parse($borrow->due_date)->isPast())
                            <form action="{{ route('admin.borrowings.overdue', $borrow->id) }}" method="POST">
                              @csrf
 ...
 <div class="card" style="margin-bottom:5px; margin-top:10px">
    ...
    <div class="card-body">
      <form id="fines-financial-form" action="{{ route('admin.financial-transactions.store') }}" method="POST" style="display:grid; grid-template-columns: repeat(4, 1fr); gap:8px;">
        @csrf
...
<div class="form-group" style="grid-column: span 2;">
 ...
 amountInput?.addEventListener('input', syncFinePaymentState);
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:0;">
          <table>
            ...
            <tbody>
              @forelse($fineRows as $row)
                <tr>
                  <td>{{ $row->transaction_type }}</td>
                  <td>
                    @php $adminDisplay = $row->patron_name ?? null; @endphp
                    @if(! empty($adminDisplay))
                      <strong>{{ $adminDisplay }}</strong>
                      @if(! empty($row->user_id))<br><small class="text-muted">{{ $row->user_id }}</small>@endif
                    @else
                      {{ $row->user_id ?? 'Library Fund' }}
                    @endif
                  </td>
                  ...
                  <td>
                    @if($row->transaction_type === 'Fine')
                      <div>Received: PHP {{ number_format((float) ($row->received_amount ?? 0), 2) }}</div>
                      <div>Change: PHP {{ number_format((float) ($row->change_amount ?? 0), 2) }}</div>
                    @else
                      <span class="status-chip {{ $row->status === 'Paid' ? 'returned' : 'pending' }}">{{ $row->status }}</span>
                    @endif
                  </td>
                  <td>{{ \Carbon\Carbon::parse($row->transaction_date)->format('M d, Y') }}</td>
                  <td style="white-space:nowrap;">
                    <!--<button class="btn-sm" type="button" onclick="populateFinancialForm('fines', {{ $row->id }}, '{{ addslashes($row->transaction_type) }}', '{{ addslashes($row->user_id ?? '') }}', '{{ addslashes($row->patron_name ?? '') }}', '{{ addslashes($row->description ?? '') }}', '{{ $row->amount }}', '{{ $row->status }}', '{{ \Carbon\Carbon::parse($row->transaction_date)->toDateString() }}')">Edit</button>
                    <form method="POST" action="{{ route('admin.financial-transactions.archive', $row->id) }}" style="display:inline-block; margin-left:6px;">
                      @csrf
    ...
    <div id="panel-funds" class="panel">
      ...
      <div class="stats-grid">
      <!-- <div class="stat-card gold"><div class="stat-label">Fund Income</div><div class="stat-value">PHP {{ number_format($fundIncome, 2) }}</div><div class="stat-change">Total fund income</div></div> -->
        <div class="stat-card blue"><div class="stat-label">Fund Expenses</div><div class="stat-value">PHP {{ number_format($fundExpense, 2) }}</div><div class="stat-change">Total fund expenses</div></div>
    <div class="stat-card green"><div class="stat-label"> Available Funds </div><div class="stat-value">PHP {{ number_format($availableFunds, 2) }}</div><div class="stat-change">Income minus expenses</div></div>
        <!--<div class="stat-card purple"><div class="stat-label">Fund Utilization</div><div class="stat-value">{{ $fundUtilization }}%</div><div class="stat-change">Expense vs income</div></div>-->
      </div>
      <div class="card" style="margin-bottom:18px;">
        <div class="card-header"><div class="card-title">Manage Funds</div></div>
        <div class="card-body">
          <form id="funds-financial-form" action="{{ route('admin.financial-transactions.store') }}" method="POST" style="display:grid; grid-template-columns: repeat(4, 1fr); gap:12px;">
            @csrf
            ...
            <div class="form-group"><label>Date</label><input type="date" name="transaction_date" value="{{ now()->toDateString() }}" required></div>
            ...
            <tbody>
              @forelse($fundRows as $row)
                <tr>
                  ...
                  <td>{{ $row->description }}</td>
                  <td style="font-weight:700;">PHP {{ number_format($row->amount, 2) }}</td>
                  <td><span class="status-chip {{ in_array($row->status, ['Paid','Available']) ? 'returned' : 'pending' }}">{{ $row->status }}</span></td>
                  <td>{{ \Carbon\Carbon::parse($row->transaction_date)->format('M d, Y') }}</td>
                  <td style="white-space:nowrap;">
                    <button class="btn-sm" type="button" onclick="populateFinancialForm('funds', {{ $row->id }}, '{{ addslashes($row->transaction_type) }}', '{{ addslashes($row->user_id ?? '') }}', '{{ addslashes($row->patron_name ?? '') }}', '{{ addslashes($row->description ?? '') }}', '{{ $row->amount }}', '{{ $row->status }}', '{{ \Carbon\Carbon::parse($row->transaction_date)->toDateString() }}')">Edit</button>
                    <form method="POST" action="{{ route('admin.financial-transactions.archive', $row->id) }}" style="display:inline-block; margin-left:6px;">
                      @csrf
                      <button class="btn-sm danger" type="submit" onclick="return confirm('Archive this transaction?')">Archive</button>
                    </form>
    ...
    <div id="panel-wifi" class="panel">
      ...
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="card-body">
          <form action="{{ route('admin.wifi-vouchers.store') }}" method="POST" style="display:grid; grid-template-columns:1fr 180px 220px auto; gap:12px; align-items:end;">
            @csrf
      ...
      <div class="stats-grid">
        <div class="stat-card green"><div class="stat-label">Available</div><div class="stat-value">{{ $wifiRows->where('status', 'Available')->count() }}</div></div>
        <div class="stat-card blue"><div class="stat-label">Used</div><div class="stat-value">{{ $wifiRows->where('status', 'Used')->count() }}</div></div>
        <div class="stat-card red"><div class="stat-label">Expired</div><div class="stat-value">{{ $wifiRows->where('status', 'Expired')->count() }}</div></div>
        <div class="stat-card gold"><div class="stat-label">Total Vouchers</div><div class="stat-value">{{ $wifiRows->count() }}</div><div class="stat-change">{{ $inactiveCount }} inactive</div></div>
      </div>
      <div class="card">
        <div class="card-body" style="padding:0;">
          <table>
            ...
            <tbody>
              @forelse($wifiRows as $voucher)
                <tr>
                  <td><code>{{ $voucher->voucher_code }}</code></td>
                  <td>{{ $voucher->name ?? 'Unassigned' }}<br><small>{{ $voucher->user_id }}</small></td>
                  <td><span class="status-chip {{ $voucher->status === 'Available' ? 'returned' : ($voucher->status === 'Expired' ? 'overdue' : 'borrowed') }}">{{ $voucher->status }}</span></td>
                  <td>
                    @if(\Illuminate\Support\Facades\Schema::hasColumn('wifi_vouchers', 'is_active'))
                      <span class="status-chip {{ $voucher->is_active ? 'returned' : 'pending' }}">{{ $voucher->is_active ? 'Active' : 'Inactive' }}</span>
                    @else
                      <span class="status-chip returned">Active</span>
                    @endif
                  </td>
                  <td>{{ $voucher->duration_minutes }} minutes</td>
                  <td>
                    @if($voucher->expires_at) Expires {{ \Carbon\Carbon::parse($voucher->expires_at)->format('M d, Y h:i A') }}<br>@endif
                    @if($voucher->used_at) Used {{ \Carbon\Carbon::parse($voucher->used_at)->format('M d, Y h:i A') }} @endif
                  </td>
                  <td>
                    <form action="{{ route('admin.wifi-vouchers.update', $voucher->id) }}" method="POST" style="display:grid; gap:8px; min-width:220px;">
                      @csrf
                      <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:6px;">
                        <select name="status">
                          <option {{ $voucher->status === 'Available' ? 'selected' : '' }}>Available</option>
                          <option {{ $voucher->status === 'Used' ? 'selected' : '' }}>Used</option>
                          <option {{ $voucher->status === 'Expired' ? 'selected' : '' }}>Expired</option>
                        </select>
                        <select name="is_active">
                          <option value="1" {{ ($voucher->is_active ?? true) ? 'selected' : '' }}>Active</option>
                          <option value="0" {{ ($voucher->is_active ?? true) ? '' : 'selected' }}>Inactive</option>
                        </select>
                        <input type="number" name="duration_minutes" min="15" value="{{ $voucher->duration_minutes }}" placeholder="Minutes">
                        <input type="datetime-local" name="expires_at" value="{{ $voucher->expires_at ? \Carbon\Carbon::parse($voucher->expires_at)->format('Y-m-d\TH:i') : '' }}">
                      </div>
                      <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:6px;">
              ...
              <tbody>
                @if(\Illuminate\Support\Facades\Schema::hasTable('proposals'))
                  @forelse($pendingProposals as $prop)
                    ...
                    @endphp
                    <tr id="proposal-row-{{ $prop->id }}">
                      <td><b>{{ $researcherName }}</b></td>
                      <td>
                        {{ $prop->title }}
                        @if($proposalDocs->isNotEmpty())
                          <br>
                          @foreach($proposalDocs as $doc)
                            <a href="{{ route('proposal.documents.view', $doc->id) }}" target="_blank" rel="noopener" style="font-size:11px; margin-right:6px;">{{ strtoupper(str_replace('_', ' ', $doc->document_type)) }}</a>
                          @endforeach
                        @endif
                        @if(!empty($prop->feedback))
                          <br><small style="color:var(--muted);">Feedback: {!! nl2br(e($prop->feedback)) !!}</small>
                        @endif
                      </td>
                      <td>{{ \Carbon\Carbon::parse($prop->deadline)->format('M d, Y') }}</td>
                      <td>₱{{ number_format($prop->budget, 2) }}</td>
                      <td><span class="status-chip pending" id="prop-badge-{{ $prop->id }}">{{ ucfirst($prop->status ?? 'Pending') }}</span></td>
                      <td style="text-align: right;">
                        <div style="display:inline-flex; gap:6px; align-items:start;">
                          <button type="button" class="btn-sm" data-inspector-id="{{ $prop->id }}" data-inspector-researcher="{{ $researcherName }}" data-inspector-title="{{ $prop->title }}" data-inspector-budget="₱{{ number_format($prop->budget, 2) }}" data-inspector-deadline="{{ \Carbon\Carbon::parse($prop->deadline)->format('M d, Y') }}" data-inspector-requirements="{{ $prop->requirements ?? '' }}" data-inspector-status="{{ ucfirst($prop->status ?? 'Pending') }}" data-inspector-feedback="{{ $prop->feedback ?? '' }}" data-inspector-docs-url="{{ route('admin.proposals.pdf', $prop->id) }}" onclick="evaluateProposalInspector(this)">View</button>
                          <form action="{{ route('admin.proposals.archive', $prop->id) }}" method="POST" class="confirmable" data-confirm="Archive this proposal?" style="display:inline;">
                            @csrf
                            <button class="btn-sm" type="submit">Archive</button>
                          </form>
                          <form action="{{ route('admin.proposals.status', $prop->id) }}" method="POST" class="confirmable" data-confirm="Approve this proposal?" style="display:grid; gap:6px;">
                            @csrf
                            <input type="hidden" name="status" value="Approved">
                            <input type="number" name="approved_grant" value="{{ $prop->budget }}" step="0.01" min="0" style="width:110px;">
                            <button class="btn-sm success" type="submit">Approve</button>
                          </form>
                          <form action="{{ route('admin.proposals.status', $prop->id) }}" method="POST" style="display:grid; gap:6px;" data-proposal-id="{{ $prop->id }}">
                            @csrf
        ...
        <div class="card">
          ...
          <div class="card-body" style="padding:0;">
            <table>
              ...
              <tbody>
                @if(\Illuminate\Support\Facades\Schema::hasTable('proposals'))
                  @forelse($approvedProposals as $prop)
                    @php
                      $proposalDocs = \Illuminate\Support\Facades\Schema::hasTable('proposal_documents')
                        ? \Illuminate\Support\Facades\DB::table('proposal_documents')->where('proposal_id', $prop->id)->get()
                        : collect();
                    ...
                    <tr id="proposal-row-{{ $prop->id }}">
                      ...
                      <td>{{ \Carbon\Carbon::parse($prop->deadline)->format('M d, Y') }}</td>
                      <td>₱{{ number_format($prop->budget, 2) }}</td>
                      <td><span class="status-chip approved">{{ ucfirst($prop->status ?? 'Approved') }}</span></td>
                      <td style="text-align: right;">
                        <div style="display:inline-flex; gap:6px; align-items:start;">
                          <button type="button" class="btn-sm" data-inspector-id="{{ $prop->id }}" data-inspector-researcher="{{ $researcherName }}" data-inspector-title="{{ $prop->title }}" data-inspector-budget="₱{{ number_format($prop->budget, 2) }}" data-inspector-deadline="{{ \Carbon\Carbon::parse($prop->deadline)->format('M d, Y') }}" data-inspector-requirements="{{ $prop->requirements ?? '' }}" data-inspector-status="{{ ucfirst($prop->status ?? 'Pending') }}" data-inspector-feedback="{{ $prop->feedback ?? '' }}" data-inspector-docs-url="{{ route('admin.proposals.pdf', $prop->id) }}" onclick="evaluateProposalInspector(this)">View</button>
                          <form action="{{ route('admin.proposals.archive', $prop->id) }}" method="POST" class="confirmable" data-confirm="Archive this proposal?" style="display:inline;">
                ...
                @if(\Illuminate\Support\Facades\Schema::hasTable('proposals'))
                  @forelse($rejectedProposals as $prop)
                    @php
                      ...
                      $researcherName = trim((string) ($researcherName ?: $prop->user_id));
                    @endphp
                    <tr id="proposal-row-{{ $prop->id }}-rejected">
                      <td><b>{{ $researcherName }}</b></td>
                      <td>
                        {{ $prop->title }}
                        @if($proposalDocs->isNotEmpty())
        ...
        <div class="card">
          ...
          <div class="card-body" style="padding:0;">
            <table>
              ...
              <tbody>
                @if(\Illuminate\Support\Facades\Schema::hasTable('proposals'))
                  @forelse($archivedProposals as $prop)
                    @php
                      $proposalDocs = \Illuminate\Support\Facades\Schema::hasTable('proposal_documents')
                        ? \Illuminate\Support\Facades\DB::table('proposal_documents')->where('proposal_id', $prop->id)->get()
                        : collect();
                      ...
                      $researcherName = trim((string) ($researcherName ?: $prop->user_id));
                    @endphp
                    <tr id="proposal-row-{{ $prop->id }}-archived">
                      <td><b>{{ $researcherName }}</b></td>
                      <td>
                        {{ $prop->title }}
                        @if($proposalDocs->isNotEmpty())
                      ...
                      <td style="text-align: right;">
                        <div style="display:inline-flex; gap:6px; align-items:start;">
                          <button type="button" class="btn-sm" data-inspector-id="{{ $prop->id }}" data-inspector-researcher="{{ $researcherName }}" data-inspector-title="{{ $prop->title }}" data-inspector-budget="₱{{ number_format($prop->budget, 2) }}" data-inspector-deadline="{{ \Carbon\Carbon::parse($prop->deadline)->format('M d, Y') }}" data-inspector-requirements="{{ $prop->requirements ?? '' }}" data-inspector-status="{{ ucfirst($prop->status ?? 'Pending') }}" data-inspector-feedback="{{ $prop->feedback ?? '' }}" data-inspector-docs-url="{{ route('admin.proposals.pdf', $prop->id) }}" onclick="evaluateProposalInspector(this)">View</button>
                          <form action="{{ route('admin.proposals.restore', $prop->id) }}" method="POST" class="confirmable" data-confirm="Restore this proposal?" style="display:inline;">
                            @csrf
          ...
          <table>
            ...
            <tbody>
              @if(\Illuminate\Support\Facades\Schema::hasTable('announcements'))
                @forelse(\Illuminate\Support\Facades\DB::table('announcements')->get() as $announcement)
                  <tr>
                    <td><strong>{{ $announcement->title }}</strong></td>
                    <td>{{ $announcement->recipients ?? 'All Users' }}</td>
                    <td>{{ \Carbon\Carbon::parse($announcement->created_at)->format('M d, Y') }}</td>
    ...
    <div id="panel-reports" class="panel">
      ...
      <div id="rep-group-borrowing" class="card report-category-group visible" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff;">
          <div class="bar-chart">
            <div class="bar-row"><div class="bar-label">Borrowed</div><div class="bar-track"><div class="bar-fill" style="width: {{ $borrowPct }}%; background-color:#2563EB;"></div></div><div class="bar-val">{{ $borrowedMonthTotal }} <span>({{ $borrowPct }}%)</span></div></div>
            <div class="bar-row"><div class="bar-label">Returned</div><div class="bar-track"><div class="bar-fill" style="width: {{ $returnPct }}%; background-color:#16A34A;"></div></div><div class="bar-val">{{ $returnedTotal }} <span>({{ $returnPct }}%)</span></div></div>
          </div>
      ...
      <div id="rep-group-genders" class="card report-category-group visible" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff;">
          <div class="bar-chart">
            <div class="bar-row"><div class="bar-label">Female</div><div class="bar-track"><div class="bar-fill gender-female" style="width: {{ $femalePct ?? 0 }}%;"></div></div><div class="bar-val">{{ $femaleCount ?? 0 }} <span>({{ $femalePct ?? 0 }}%)</span></div></div>
            <div class="bar-row"><div class="bar-label">Male</div><div class="bar-track"><div class="bar-fill gender-male" style="width: {{ $malePct ?? 0 }}%;"></div></div><div class="bar-val">{{ $maleCount ?? 0 }} <span>({{ $malePct ?? 0 }}%)</span></div></div>
          </div>
          <table style="margin-top:14px; font-size:12px;"><thead><tr><th>Gender</th><th class="text-center">Count</th><th class="text-center">Percentage</th></tr></thead><tbody><tr><td>Female</td><td class="text-center">{{ $femaleCount ?? 0 }}</td><td class="text-center">{{ $femalePct ?? 0 }}%</td></tr><tr><td>Male</td><td class="text-center">{{ $maleCount ?? 0 }}</td><td class="text-center">{{ $malePct ?? 0 }}%</td></tr></tbody></table>
        </div>
      </div>
      ...
      <div id="rep-group-schools" class="card report-category-group" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff;">
          <div class="bar-chart">
            @forelse(($schoolDistribution ?? collect())->take(8) as $school)
              @php $schoolPct = ($totalSchoolUsers ?? 0) > 0 ? round(($school->count / $totalSchoolUsers) * 100) : 0; @endphp
              <div class="bar-row"><div class="bar-label">{{ $school->school ?: 'Unspecified' }}</div><div class="bar-track"><div class="bar-fill {{ $schoolColorClass($school->school) }}" style="width: {{ $schoolPct }}%;"></div></div><div class="bar-val">{{ $school->count }} <span>({{ $schoolPct }}%)</span></div></div>
            @empty
          ...
          <table style="margin-top:14px; font-size:12px;"><thead><tr><th>Name</th><th>School</th><th class="text-center">Count</th></tr></thead><tbody>
            @forelse(($schoolDistribution ?? collect())->take(10) as $school)
              <tr><td>—</td><td>{{ $school->school ?: 'Unspecified' }}</td><td class="text-center">{{ $school->count }}</td></tr>
            @empty
      ...
      <div id="rep-group-ages" class="card report-category-group" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff;">
          <div class="bar-chart">
            @foreach(($ageBrackets ?? []) as $label => $count)
              @php $agePct = ($totalAgeUsers ?? 0) > 0 ? round(($count / $totalAgeUsers) * 100) : 0; @endphp
              <div class="bar-row"><div class="bar-label">{{ $label }}</div><div class="bar-track"><div class="bar-fill {{ $ageColorClass($label) }}" style="width: {{ $agePct }}%;"></div></div><div class="bar-val">{{ $count }} <span>({{ $agePct }}%)</span></div></div>
            @endforeach
          </div>
          <table style="margin-top:14px; font-size:12px;"><thead><tr><th>Age Bracket</th><th class="text-center">Count</th><th class="text-center">Percentage</th></tr></thead><tbody>
            @php $ageTotal = max(1, array_sum($ageBrackets ?? [])); @endphp
            @foreach(($ageBrackets ?? []) as $label => $count)
              <tr><td>{{ $label }}</td><td class="text-center">{{ $count }}</td><td class="text-center">{{ round(($count / $ageTotal) * 100) }}%</td></tr>
            @endforeach
          </tbody></table>
        </div>
      </div>
      ...
      <div id="rep-group-proposals" class="card report-category-group" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff;">
          <div class="bar-chart">
            @foreach(['pending' => ['Pending', '#F59E0B'], 'approved' => ['Approved', '#16A34A'], 'rejected' => ['Rejected', '#EF4444']] as $key => [$label, $color])
              @php $proposalPct = round((($proposalStats[$key] ?? 0) / $proposalTotal) * 100); @endphp
              <div class="bar-row"><div class="bar-label">{{ $label }}</div><div class="bar-track"><div class="bar-fill" style="width: {{ $proposalPct }}%; background-color: {{ $color }};"></div></div><div class="bar-val">{{ $proposalStats[$key] ?? 0 }} <span>({{ $proposalPct }}%)</span></div></div>
            @endforeach
          </div>
      ...
      <div id="rep-group-topusers" class="card report-category-group" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff; padding:0;">
          <table><thead><tr><th>Rank</th><th>Visitor Name</th><th>Barcode</th><th>Visit Count</th></tr></thead><tbody>
            @forelse(($topVisitors ?? collect())->take(10) as $index => $visitor)
              <tr><td><span class="rank-num turquoise-rank">{{ $index + 1 }}</span></td><td>{{ $visitor->full_name ?? 'Unknown' }}</td><td>{{ $visitor->barcode_id ?? 'N/A' }}</td><td>{{ $visitor->visit_count ?? 0 }}</td></tr>
            @empty
      ...
      <div id="rep-group-topwifi" class="card report-category-group" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff; padding:0;">
          <table><thead><tr><th>Rank</th><th>User</th><th>User ID</th><th>Bandwidth</th><th>Voucher</th></tr></thead><tbody>
            @forelse(($topWifiUsers ?? collect())->take(10) as $index => $wifiUser)
              <tr><td><span class="rank-num turquoise-rank">{{ $index + 1 }}</span></td><td>{{ $wifiUser->name ?? 'Unknown' }}</td><td>{{ $wifiUser->user_id ?? 'N/A' }}</td><td>{{ $wifiUser->bandwidth_gb ?? 0 }} GB</td><td>{{ $wifiUser->voucher_code ?? 'N/A' }}</td></tr>
            @empty
      ...
      <div id="rep-group-gatecheckins" class="card report-category-group" style="margin-bottom: 20px;">
        ...
        <div class="card-body" style="background:#ffffff; padding:0;">
          <table>
            ...
            <tbody>
              @forelse(($gateEntries ?? collect())->take(20) as $entry)
                <tr><td>{{ $entry->full_name ?? 'Unknown Member' }}</td><td>{{ $entry->barcode_id ?? 'N/A' }}</td><td>{{ $entry->entry_time ? \Carbon\Carbon::parse($entry->entry_time)->format('M d, Y h:i A') : 'N/A' }}</td><td>{{ $entry->exit_time ? \Carbon\Carbon::parse($entry->exit_time)->format('M d, Y h:i A') : 'Active' }}</td></tr>
              @empty
  ...
  <div class="utilities-grid">
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:24px;">
        <form action="{{ route('admin.users.store') }}" method="POST" id="create-user-form" novalidate>
          @csrf
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:24px;">
        <form action="{{ route('admin.users.status') }}" method="POST" id="update-status-form">
          @csrf
          <div class="form-group">
        ...
        @error('user_status_error')
          <div style="color:#dc2626; font-size:12px; margin-top:10px;">{{ $message }}</div>
        @enderror
      </div>
    </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select name="target_user_id" id="admin-user-reset-select" onchange="populateResetUserForm()">
                  ...
                  @foreach(App\Models\User::orderBy('role')->orderBy('name')->get() as $account)
                    <option value="{{ $account->id }}" data-user-id="{{ $account->user_id }}" data-name="{{ $account->name }}">
                      {{ $account->name }} ({{ ucfirst($account->role) }})
                    </option>
                  @endforeach
                </select>
              </div>
  ...
  <div class="utilities-grid utilities-grid-wide">
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:24px;">
        <form action="{{ route('admin.schools.store') }}" method="POST" style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:16px;">
          @csrf
        ...
        @if(\Illuminate\Support\Facades\Schema::hasTable('school_options'))
          @php $managedSchools = \Illuminate\Support\Facades\DB::table('school_options')->orderBy('name')->get(); @endphp
          @if($managedSchools->isNotEmpty())
            <div style="display:flex; flex-wrap:wrap; gap:8px;">
              @foreach($managedSchools as $school)
                <div style="display:flex; align-items:center; gap:8px; padding:8px 12px; border:1px solid #d1d5db; border-radius:999px; background:#f8fafc; font-size:13px;">
                  ...
                  <form action="{{ route('admin.schools.delete', $school->id) }}" method="POST" style="margin:0; display:inline;">
                    @csrf
  ...
  <div class="utilities-grid utilities-grid-wide">
    <div class="card">
      ...
      <div class="card-body" style="padding:24px;">
        <form action="{{ route('admin.backups.store') }}" method="POST" style="display:grid; gap:12px;">
          @csrf
        ...
        @if(\Illuminate\Support\Facades\Schema::hasTable('backup_logs'))
          @php $backups = \Illuminate\Support\Facades\DB::table('backup_logs')->latest()->limit(5)->get(); @endphp
          <div style="margin-top:14px; display:grid; gap:8px;">
            @forelse($backups as $backup)
              <div style="display:flex; justify-content:space-between; gap:8px; border:1px solid #e2e8f0; border-radius:10px; padding:10px 12px; background:#f8fafc;">
                <div>
                  <div style="font-weight:600; color:#0f172a; font-size:13px;">{{ $backup->filename }}</div>
                  <div style="font-size:11px; color:#64748b;">{{ \Carbon\Carbon::parse($backup->created_at)->format('M d, Y h:i A') }}</div>
                </div>
                <div style="font-size:12px; color:#0f172a; font-weight:700;">{{ number_format($backup->size / 1024, 2) }} KB</div>
              </div>
            @empty
      ...
      <div class="card">
        <div class="card-body" style="padding:0;">
          <table>
            ...
            <tbody>
              @if(\Illuminate\Support\Facades\Schema::hasTable('backup_logs'))
                @forelse(\Illuminate\Support\Facades\DB::table('backup_logs')->latest()->get() as $backup)
                  <tr><td>{{ $backup->filename }}</td><td>{{ number_format($backup->size / 1024, 2) }} KB</td><td>{{ \Carbon\Carbon::parse($backup->created_at)->format('M d, Y h:i A') }}</td><td><span class="status-chip returned">Stored local</span></td></tr>
                @empty
...
<script>
  ...
  function exportFilteredReportsPDF() {
    ...
    const url = '{{ route("admin.reports.export") }}?categories=' + encodeURIComponent(categories) + '&view=1';
