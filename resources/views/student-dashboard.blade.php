...
@section('title', 'Panabo City Library - Student Portal')
...
@section('content')
  <section id="panel-dashboard" class="panel active">
    <div class="grid-3">
      @include('components.stat-card', ['label' => 'Borrowed Books', 'value' => $borrowings->count(), 'change' => 'Current loans'])
      @include('components.stat-card', ['label' => 'Announcements', 'value' => $announcements->count(), 'change' => 'Latest updates'])
      @include('components.stat-card', ['label' => 'Outstanding Fines', 'value' => 'PHP ' . number_format($fines->where('status', 'Unpaid')->sum('amount') ?? 0, 2), 'change' => 'Please settle soon'])
    </div>

    <div class="grid-2">

      @if(!empty($dueSoonBorrowings) && count($dueSoonBorrowings) > 0)
        <div style="margin-bottom:16px;">
          @component('components.card', ['title' => 'Upcoming Due Reminder', 'subtitle' => "Books due within next 3 days"]) 
            <ul style="margin:0;padding-left:18px;">
              @foreach($dueSoonBorrowings as $b)
                @php $daysLeft = \Carbon\Carbon::parse($b->due_date)->diffInDays(\Carbon\Carbon::now()); @endphp
                <li style="margin-bottom:8px;"> <strong>{{ $b->title ?? $b->book_barcode }}</strong> — due in {{ $daysLeft }} day{{ $daysLeft > 1 ? 's' : '' }} ({{ \Carbon\Carbon::parse($b->due_date)->format('M d, Y') }})</li>
              @endforeach
            </ul>
          @endcomponent
        </div>
      ...
      @component('components.card', ['title' => 'Latest Announcements'])
  ...
  <section id="panel-borrowings" class="panel">
    ...
    @component('components.card', ['title' => 'Request History', 'subtitle' => 'Track your request status from submission to staff action'])
      <table>
        <thead><tr><th>Book</th><th>Requested At</th><th>Status</th><th>Rejection Reason</th></tr></thead>
        <tbody>
          @forelse($requestHistory as $request)
            <tr>
              <td>{{ $request->title ?? $request->accession_number ?? 'Unknown book' }}</td>
              <td>{{ \Carbon\Carbon::parse($request->created_at)->format('M d, Y h:i A') }}</td>
              <td><span class="status-chip {{ strtolower($request->status) === 'approved' || strtolower($request->status) === 'borrowed' ? 'approved' : (strtolower($request->status) === 'rejected' ? 'danger' : 'pending') }}">{{ ucfirst($request->status ?? 'Pending') }}</span></td>
              <td>
  ...
  <section id="panel-fines" class="panel">
    @component('components.card', ['title' => 'Fines History'])
      <table>
        ...
        <tbody>
          @forelse($fines as $fine)
            <tr>
              ...
              <td><span class="status-chip {{ $fine->status === 'Paid' ? 'returned' : 'pending' }}">{{ $fine->status }}</span></td>
        ...
        <table>
          ...
          <tbody>
            @foreach($notifications as $notification)
              <tr>
                <td>{{ $notification->data['message'] ?? $notification->data['title'] ?? $notification->data['body'] ?? 'Notification' }}</td>
  ...
  <section id="panel-opac" class="panel">
    @component('components.card', ['title' => 'Browse & Request Books', 'subtitle' => 'Search the library catalog and request books to borrow'])
  ...
  @if($isRegularUser)
    <section id="panel-barcode-request" class="panel">
      @component('components.card', ['title' => 'Request New Barcode', 'subtitle' => 'Request a replacement barcode if yours is lost or damaged'])
        <div style="margin-bottom: 20px;">
          <form method="POST" action="{{ route('student.barcode-request.store') }}" enctype="multipart/form-data" style="display: grid; grid-template-columns: 1fr; gap: 12px; margin-bottom: 20px;">
          @csrf
          ...
          <div>
            ...
            @if($barcodeRequests && count($barcodeRequests) > 0)
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                  @forelse($barcodeRequests as $request)
                    <tr style="border-bottom: 1px solid #e5e7eb;">
                      <td style="padding: 14px 14px; font-size: 13px; color: #1f2937;">{{ $request->old_barcode_id ?? '—' }}</td>
                      <td style="padding: 14px 14px; font-size: 13px; color: #1f2937;">
                        <div>{{ ucfirst($request->reason) }}</div>
                      ...
                      <td style="padding: 14px 14px; font-size: 13px; color: #1f2937;">
                        <span style="display: inline-flex; align-items: center; gap: 5px; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; background: {{ $request->status === 'approved' ? 'rgba(34, 197, 94, 0.12); color: #22c55e' : ($request->status === 'pending' ? 'rgba(243, 156, 18, 0.15); color: #f59e0b' : 'rgba(239, 68, 68, 0.12); color: #ef4444') }};">
                          {{ ucfirst($request->status) }}
                        </span>
                      </td>
                      <td style="padding: 14px 14px; font-size: 13px; color: #1f2937; font-weight: 600;">{{ $request->new_barcode_id ?? '—' }}</td>
                      <td style="padding: 14px 14px; font-size: 13px; color: #6b7280;">{{ \Carbon\Carbon::parse($request->created_at)->format('M d, Y') }}</td>
                    </tr>
                  @empty
                    <tr>
