...
@section('title', 'Proposal Review - Panabo City Library')
...
@section('content')
    ...
    <section class="proposal-review-document">
      <div class="proposal-review-header">
        ...
        <div class="proposal-review-actions">
          ...
          <a id="doc-download" href="{{ route('proposal.documents.download', $selected->id) }}" class="btn-sm">Download</a>
        </div>
      </div>
      ...
      <div class="proposal-document-tabs">
        @foreach($documents as $doc)
          <a
            class="proposal-document-tab {{ $doc->id === $selected->id ? 'active' : '' }}"
            href="{{ route('proposal.documents.view', $doc->id) }}"
            target="_blank"
            rel="noopener"
          >
      ...
      <div id="pdf-container" class="proposal-document-frame">
        ...
        @elseif($isImage)
          <img src="{{ $selectedUrl }}" alt="{{ $selected->original_name }}" class="proposal-image-preview">
        @else
        ...
        @forelse($comments as $c)
          <div class="comment-item">
            <div class="comment-meta">
              <strong>{{ ucfirst($c->commenter_role ?? 'Admin') }}</strong>
  ...
  @if($isPdf)
    <script>
      ...
      const existingComments = @json($comments->toArray());
      let pdfDoc = null;
      let scale = 1.25;
      ...
      loadPdf(@json($selectedUrl)).catch((error) => {
        console.error(error);
  ...
  <script>
    document.getElementById('comment-form').addEventListener('submit', async function (event) {
      ...
      const response = await fetch('{{ route('admin.proposals.comments', $proposal->id) }}', {
        method: 'POST',
