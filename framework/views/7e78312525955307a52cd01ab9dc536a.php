...
<head>
  ...
  <title><?php echo e($book->title); ?> — OPAC</title>
  <style>
    :root {
      --bg: #f7fafc;
      --card: #ffffff;
...
<body>
  <div class="page">
    <div class="breadcrumbs">
      <a href="<?php echo e(route('opac.index')); ?>">OPAC</a> / <?php echo e($book->title); ?>
    ...
    <div class="detail-grid">
      <div class="book-panel book-cover">
        ...
        <?php else: ?>
          ...
          ?>
          <div class="book-cover-fallback" style="background: linear-gradient(135deg, <?php echo e($colors[0]); ?>, <?php echo e($colors[1]); ?>);">
      ...
      <div class="book-info">
        <div>
          ...
          <div class="status-pill <?php echo e(($book->available ?? 0) > 0 ? 'status-available' : 'status-borrowed'); ?>">
            <?php echo e(($book->available ?? 0) > 0 ? 'Available' : 'Unavailable'); ?>

          </div>
        </div>
        <div class="summary">
          <?php echo e($book->summary ?? 'No summary description is available for this title.'); ?>

        </div>
        <div class="meta-block">
          <div class="meta-card">
            <div class="meta-label">Accession</div>
            <div class="meta-value"><?php echo e($book->accession_number); ?></div>
            <div class="meta-text">Neat catalog identifier for shelving and lookup.</div>
          </div>
          <div class="meta-card">
            <div class="meta-label">Book Barcode</div>
            <div style="display: flex; flex-direction: column; gap: 8px;">
              <img src="<?php echo e(route('barcode.book.png', ['barcode' => $book->barcode])); ?>" alt="Barcode: <?php echo e($book->barcode); ?>" style="height: 50px; width: 100%; object-fit: contain;">
              <div class="meta-value" style="font-family: monospace; font-size: 12px;"><?php echo e($book->barcode); ?></div>
            </div>
          </div>
          <div class="meta-card">
            <div class="meta-label">Location</div>
            <div class="meta-value"><?php echo e($book->section_location ?? 'General collection'); ?></div>
          </div>
          <div class="meta-card">
            <div class="meta-label">Inventory</div>
            <div class="meta-value"><?php echo e($book->available ?? 0); ?> of <?php echo e($book->copies ?? 0); ?> available</div>
          </div>
        </div>
        <div class="actions">
          <a class="btn btn-secondary" href="<?php echo e(route('opac.index')); ?>">Back to Catalog</a>
        <!--  <a class="btn btn-primary" href="javascript:window.scrollTo(0,document.body.scrollHeight);">Read Details</a> -->
        </div>
      </div>
      <?php if(auth()->guard()->check()): ?>
        <?php if(in_array(auth()->user()->role, ['student', 'visitor'])): ?>
          <div class="actions">
              <button class="btn btn-primary" onclick="requestBook(<?php echo e($book->id); ?>)">Request Book</button>
          </div>
        <?php endif; ?>
      <?php else: ?>
        <div class="actions">
          ...
          <div class="modal-body">
            <div class="login-card">
              ...
              <div class="login-actions">
                ...
                <a href="<?php echo e(route('login')); ?>">Full login page</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <?php if(auth()->guard()->check()): ?>
        <?php if(in_array(auth()->user()->role, ['student', 'visitor'])): ?>
          ...
          async function requestBook(bookId) {
              ...
              fetch('<?php echo e(route("opac.request-borrow")); ?>', {
                  method: 'POST',
                  headers: {
  ...
  <script>
    ...
    function submitModalLogin() {
      ...
      fetch('<?php echo e(route('login.ajax')); ?>', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php /**PATH C:\laragon\www\LIBRARY-MANAGEMENT-SYSTEM\resources\views/opac-book.blade.php ENDPATH**/ ?>